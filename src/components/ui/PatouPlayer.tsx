...
interface PatouPlayerProps {
  className?: string
  onTrackChange?: (trackId: string) => void
}

export default function PatouPlayer({ className = '', onTrackChange }: PatouPlayerProps) {
  const {
    currentTrack,
    isPlaying,
    position,
    ...
    <div className={`bg-gradient-to-br from-gray-900 to-black text-white rounded-xl p-6 ${className}`}>
      ...
      {currentTrack ? (
        <div className="flex items-center gap-4 mb-6">
          ...
          >
            <Heart className="w-5 h-5 text-share" />
          </button>
        </div>
      ) : (
      ...
      <div className="flex items-center justify-center gap-6 mb-6">
        ...
        <button
          ...
          disabled={!isReady}
          className="bg-gradient-to-r from-primary to-primary-glow hover:opacity-90 disabled:bg-gray-600 text-white rounded-full p-4 transition-all transform hover:scale-105"
        >
      ...
      <div className="flex items-center gap-3">
        ...
        <input
          ...
          style={{
            background: `linear-gradient(to right, hsl(var(--primary)) 0%, hsl(var(--primary)) ${volume * 100}%, #374151 ${volume * 100}%, #374151 100%)`
          }}
        />
